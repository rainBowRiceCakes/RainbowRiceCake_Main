...
export const uploadPickupPhoto = createAsyncThunk(
  ...
  async ({ orderId, formData }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(`/api/orders/${orderId}/pickup-photo`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data', // 브라우저가 boundary를 자동으로 붙이도록 유도
        },
      });
      return { orderId, data: response.data }; // 서버가 바뀐 주문 객체나 성공여부를 반환한다고 가정
    } catch (err) {
...
export const uploadCompletePhoto = createAsyncThunk(
  ...
  async ({ orderId, formData }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(`/api/orders/${orderId}/complete-photo`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data', // 브라우저가 boundary를 자동으로 붙이도록 유도
        },
      });
